document.addEventListener('DOMContentLoaded', () => {
    ...
    function initAdvancedPage() {
        ...
        function performUndo() {
            ...
            // Restore Strip Pairs
            stripPairs = JSON.parse(JSON.stringify(previousState.stripPairs)); 
            // Update UI values
            const stripsVal = document.getElementById('strips-value');
            if (stripsVal) stripsVal.textContent = previousState.settings.strips;
            if (document.getElementById('pair-boundary-start-value')) document.getElementById('pair-boundary-start-value').textContent = previousState.settings.boundaryStart;
            if (document.getElementById('pair-boundary-end-value')) document.getElementById('pair-boundary-end-value').textContent = previousState.settings.boundaryEnd;
        ...
        function initAdvancedUploader() {
            ...
            if (multiUploadInput) multiUploadInput.addEventListener('change', (e) => {
                if (e.target.files) processFiles(e.target.files);
            });
        }

        function processFiles(files) {
            saveHistory("Nahrání nových obrázků"); 
            const imageStatus = document.getElementById('image-status');
            previewsContainer.innerHTML = '';
        ...
        function updateStripPairEditorUI() {
            ...
            if (stripPairs.length === 0) stripPairs.push({
                top: 0,
                bottom: 0
            });
        ...
        function updateStripPairsState() {
            ...
            document.querySelectorAll('#strip-pairs-editor > div').forEach(wrapper => {
                newStripPairs.push({
                    top: parseInt(wrapper.querySelector('select[data-half="top"]').value, 10),
                    bottom: parseInt(wrapper.querySelector('select[data-half="bottom"]').value, 10)
                });
            });
